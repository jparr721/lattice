...
void Spring::ComputeVertexPoints() {
    ...
    const auto v1 = Eigen::Vector3f(lpos(utility::VectorPosition::X), lpos(utility::VectorPosition::Y), 0.f);
    const auto v2 = Eigen::Vector3f(rpos(utility::VectorPosition::X), rpos(utility::VectorPosition::Y), 0.f);
    const auto v3 =
        Eigen::Vector3f(rpos(utility::VectorPosition::X) + 0.01f, rpos(utility::VectorPosition::Y), 0.f);
...
void Spring::ApplySpringForces() {
    ...
    force = stiffness * (current_spring_length - rest_length) *
            mass_norm.array();
